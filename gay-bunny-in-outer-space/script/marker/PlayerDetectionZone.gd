...
func update() -> void:
	...
	if !active:
		...
		if distance < activate_distance:
			activate.emit(player, player.possessed_control_entity)
			active = true
			has_been_activated = true
	elif active:
		if distance > deactivate_distance:
			deactivate.emit(player, player.possessed_control_entity)
			active = false
...
func get_player_distance() -> float:
	...
	if !is_instance_valid(player.possessed_control_entity):
		return 1000000000

	return (global_position - player.possessed_control_entity.global_position).length()
